...
export const PopulationState: React.FC<PopulationStateProps> = ({
  ...
  const applyState = useCallback((updated: PopulationStateData, transport: 'http' | 'websocket', estimatedServerTime?: number) => {
    traceStateUpdate(updated, transport, () => {
      ...
      onUpdate?.(updated, estimatedServerTime);
    });
  }, [onUpdate]);
  ...
  const fetchHttpState = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        applyState(updated, 'http', typeof json.server_time_ms === 'number'
